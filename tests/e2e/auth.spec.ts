...
test('sessao expirada durante a navegacao encerra a sessao local', async ({ page }) => {
  ...
  await page.getByRole('button', { name: /Fechar aviso de sessao expirada/i }).click()
  await expect(page.getByRole('status').filter({ hasText: /Sua sessao expirou/i })).toBeHidden()

  await setScenario(page, { forceSessionExpired: false })
  await loginByApi(page)
  await setScenario(page, { forceSessionExpired: true })
  await page.goto('/carrinho')
  await page.locator('#coupon:visible, #coupon-mobile:visible').fill('KURIO10')
